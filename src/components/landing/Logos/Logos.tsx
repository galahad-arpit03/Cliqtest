...
export default function Logos({ isLight }: { isLight?: boolean }) {
  ...
  return (
    <section
      className={`w-full py-6 md:py-8 border-y overflow-hidden transition-colors duration-500 ${isLight ? "bg-[#FAFAFA] border-app-bg/5" : "bg-[#000000] border-app-border"}`}
    >
      <motion.div
      ...
      >
        <div className="inline-flex flex-col items-center">
          <h2
            className={`text-sm md:text-base font-medium tracking-[0.2em] uppercase transition-colors duration-500 text-[#6843B7] pb-2`}
          >
            ...
            {duplicatedIntegrations.map((item, i) => (
              ...
              >
                ...
                <div className="flex flex-col text-left justify-center">
                  <h4
                    className={`font-bold text-sm transition-colors leading-tight ${isLight ? "text-gray-500" : "text-gray-400"}`}
                  >
                    {item.name}
                  </h4>
                </div>
