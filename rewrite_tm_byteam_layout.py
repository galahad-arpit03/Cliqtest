...
export default function TestManagementFeatures() {
  ...
  return (
    <div className="bg-[#050505] text-white min-h-screen relative overflow-hidden">
      ...
      <div className="absolute top-0 left-0 w-full h-full overflow-hidden pointer-events-none opacity-30 z-0 fixed">
         <div className="absolute top-1/4 left-0 w-[40%] h-[40%] rounded-full bg-[#2563EB]/20 blur-[150px]" />
         <div className="absolute bottom-1/4 right-0 w-[50%] h-[50%] rounded-full bg-white/5 blur-[150px]" />
      </div>

      {/* Main Layout: Sticky Sidebar + Scrolling Content */}
      <section className="relative z-10 max-w-7xl mx-auto px-8 py-20 flex flex-col lg:flex-row gap-16">
            ...
            {features.map(({ id, title }) => (
              <button
                ...
                className={`text-left px-4 py-3 rounded-md transition-all font-medium text-[15px] ${
                  activeSection === id 
                    ? 'bg-[#2563EB]/10 text-white font medium border border-[#2563EB]/30 shadow-[0_0_15px_rgba(37,99,235,0.1)]' 
             ...
             return (
              <div key={feat.id} id={feat.id} className="scroll-mt-32">
                <h2 className="text-3xl md:text-4xl font-medium mb-6 tracking-tight">
                  {firstWord} <span className="text-transparent bg-clip-text bg-gradient-to-r from-[#2563EB] to-[#ffffff]">{restWords}</span>
                </h2>
                
                <div className="grid grid-cols-1 md:grid-cols-2 gap-12 items-stretch mt-8">
                  <div className="flex flex-col h-full">
                    
                    <div className="space-y-6 text-white/60 leading-relaxed text-[15px] mb-10">
                      {feat.desc.map((p, i) => (
                        <p key={i}>{p}</p>
                      ))}
                    </div>
                      ...
                      <div className="grid grid-cols-1 gap-3">
                        {feat.benefits.map((benefit, i) => (
                          <div key={i} className="flex items-start gap-3 text-sm text-white/70">
                            <div className="mt-0.5 text-[#2563EB]">✓</div>
                  ...
                  <div className="bg-[#0A0A0A] border border-white/10 rounded-xl relative overflow-hidden flex flex-col justify-center h-full min-h-[350px] group">
                    <div className="absolute top-0 right-0 w-32 h-32 bg-[#2563EB] opacity-10 blur-[60px] rounded-full group-hover:opacity-30 transition-opacity duration-700" />
                    <div className="absolute bottom-0 left-0 w-32 h-32 bg-white opacity-5 blur-[60px] rounded-full group-hover:opacity-10 transition-opacity duration-700" />
                    
                    <div className="relative z-10 flex items-center justify-center">
                       <div className="relative w-40 h-40 rounded-2xl bg-white/[0.02] border border-white/5 backdrop-blur-xl shadow-2xl flex items-center justify-center hover:border-white/20 transition-all duration-500">
                           <feat.icon size={64} className="text-[#2563EB] opacity-60 group-hover:opacity-100 group-hover:scale-110 transition-all duration-700" />
                           <div className="absolute w-full h-full animate-[spin_10s_linear_infinite] pointer-events-none">
                              <div className="absolute top-0 left-0 w-2 h-2 rounded-full shadow-[0_0_10px_#60A5FA] bg-[#60A5FA]" />
                           </div>
                       </div>
                    </div>
                  </div>
...
print("Test Management UI updated to Sticky Sidebar layout")
